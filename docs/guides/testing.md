...
- `test_package_projects_prd_trd_into_dev_and_design_spec_into_design` — dev gets the full PRD + TRD and design gets the full design spec as read-only `reference/` projections (source-hash stamped), each scoped to its audience and wired into the README with a valid link (PMOS-002).
