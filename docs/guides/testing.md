...
- `test_global_stage_affinity_filters` — **(F1)** a global entry with `stages: [...]` injects only into those stages; a bare-string global still applies to every stage (back-compat).
- `test_scoped_project_global_overrides_base_bare_entry` — **(F1 fix)** a project manifest scoping a file the base declares as a bare string overrides it (globals merge by path), so the file injects only into the scoped stage, once — not twice, and not everywhere.
...
- `test_design_journey_coverage_is_scoped_to_mvp_band` — journeys inherit tier from the requirements they serve; a design spec that omits a *deferred-only* journey (traces only to `v1`/`v2`/`later`) raises no `JOURNEY_FLOW_TRACE_MISSING`, while an omitted *mvp-band* journey is still flagged (scoped, not disabled).
...
- **Tier field (F2):** `test_block_tier_defaults_parses_and_preserves_invalid` — `block_tier` reads a `Tier:` field (absent → `mvp`; valid value lower-cased; unrecognized value preserved for a validator to flag); `is_valid_tier` gates the recognized bands.
- **Tiered fidelity (NP2 / D2):** `test_non_mvp_story_is_stub_not_full_minispec` (a `v1` story is exempt from the mvp acceptance/happy-path/edge checks, and a complete SOW-grade stub raises no warning) and `test_non_mvp_stub_missing_commitment_fields_warns` (a stub missing Value/Size/Rationale/Depends-on/Acceptance-intent raises `USER_STORY_STUB_FIELDS_MISSING`, not the mvp mini-spec). Untagged stories default to `mvp`, so tier-less PRDs validate exactly as before. `test_invalid_tier_is_flagged_and_kept_on_mvp_checks` (a typo'd tier is surfaced as `USER_STORY_TIER_INVALID` and stays on the full mvp mini-spec — never silently stub-exempted). `test_non_mvp_stub_missing_traceability_warns` (a stub must keep its `Traceability` link — Codex P2). `test_functional_requirement_invalid_tier_warns` (FRs carry + are validated on their scope tier, like stories — `FUNCTIONAL_REQUIREMENT_TIER_INVALID`, Codex P1). `test_requirements_by_tier_excludes_reference_only_ids` (reference-only phantom ids are flagged `declared: False` and excluded from tier groups/counts — Codex P2).
...
- **Stage-05 prototype slice auditability:** `test_prd_journey_prototype_priority_warns_when_missing` verifies PRD journeys expose a declared prototype priority / validation-risk signal; `test_prototype_brief_warns_when_high_priority_journey_lacks_slice_decision` verifies high-priority journeys must be explicitly included or excluded in `What to Prototype`; `test_prototype_priority_check_is_scoped_to_mvp_journeys` verifies a high-priority *deferred-only* journey raises no `PROTOTYPE_PRIORITY_DECISION_MISSING` (the mvp prototype correctly omits it) while an undecided *mvp* journey still warns.
- **Delivery map mvp band (`tests/unit/test_delivery_map.py`):** `test_delivery_map_default_keeps_whole_product` (default map is every tier — the traceability spine relies on it); `test_delivery_map_mvp_only_drops_deferred_and_empty_epics` (`mvp_only=True` drops deferred stories/requirements, journeys serving only deferred requirements, and epics left with no member — so the Jira export and readable package never turn roadmap-context stubs into build tickets/items); `test_delivery_map_mvp_only_untagged_prd_is_unchanged` (back-compat: a pre-tiering PRD defaults every block to mvp, so `mvp_only` drops nothing).
- **Generated history lineage:** `test_pm_snapshot_stamps_hash_and_copies_exact_artifact` verifies `pm_snapshot.py` stamps/verifies `generated_hash` and copies the exact artifact into `.history/`; `test_pm_check_warns_when_generated_snapshot_missing_or_mismatched` verifies `/pm-check` reports warning-only lineage degradation for missing or mismatched generated snapshots; `test_pm_check_healthy_when_generated_snapshot_matches` asserts the *matching* path is clean (no `HISTORY_SNAPSHOT_HASH_MISMATCH`) — a regression guard for the missing `hash_artifact_body` import in `consistency.py` that made every snapshot hash raise `NameError`, get swallowed, and misreport as an unreadable mismatch (the mismatch-only test could not catch it).
...
- `test_uncovered_requirements_span_all_tiers_with_labels` — `/pm-trace`'s coverage report is a whole-product *inspector* (not a pipeline gate): it lists uncovered requirements across all tiers, and `uncovered_requirement_tiers` labels each with its tier (deferred shown, not hidden). Reference-only phantom ids (`declared` False) are still excluded.
- `test_query_rebuilds_a_stale_schema_index` — `_index_for_query` rebuilds when the on-disk `.traceability.yaml` predates the current schema (< 7): a legacy index lacking the `declared` field would make declared-aware queries silently wrong (falsely reporting full coverage), so a schema mismatch forces a fresh in-memory build.
...
- `test_traceability_records_requirement_tier` — **(F2)** the spine records a `tier` per requirement from each US/FR `Tier:` field (default mvp); `requirements_by_tier`/`mvp_requirements` group the whole product so the MVP slice is filterable (schema v6).
...
- `test_plan_excludes_deferred_tier_items` — the Jira export is the mvp band: a deferred (`v1`) story/requirement/epic is not exported, and a TRD task implementing *only* deferred requirements is dropped, while a task implementing any mvp requirement stays exportable.
...
- `test_package_journeys_reference_excludes_deferred_journeys` — the package's `user-journeys.md` reference renders from the mvp-only delivery map (`delivery.journey_blocks`), not the raw PRD section, so a journey serving only deferred work is not shipped in the build package.
...
**`test_promote.py`** — **(NP3)** the read-only `/pm-promote` cost preview + `pm_status` tier counts. `test_promote_preview_free_on_draft_prd` (a tier change on an unapproved PRD cascades nothing → FREE); `test_promote_preview_low_on_approved_prd_no_downstream` (approved PRD, no approved downstream → LOW); `test_promote_preview_unknown_requirement_errors` (a requirement absent from the PRD errors and writes nothing); `test_status_shows_tier_counts` (`pm_status` prints the whole-product tier breakdown); `test_promote_preview_rejects_reference_only_id` (an id only *referenced* in the PRD — no declaring block — is rejected, Codex P2); `test_promote_preview_rejects_invalid_current_tier` (a declared block with a typo'd tier errors with an actionable message instead of an uncaught ValueError, Codex P2). The actual re-tag + regeneration is the skill's job, so `pm_promote.py` never touches gate/hash/status.
...
| `test_trd_coverage_ignores_deferred_requirements` | `TRD_REQ_NOT_IMPLEMENTED` is scoped to the mvp band — a deferred (`v1`/`v2`/`later`) requirement the TRD correctly omits raises no coverage warning, while an uncovered mvp requirement still does. |
| `test_story_screen_coverage_ignores_deferred_stories` | `STORY_HAS_NO_SCREEN` is scoped to the mvp band — a deferred story with no screen raises no warning (design builds mvp screens only), while an unserved mvp story still does. |
