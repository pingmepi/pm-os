...
**`test_share_package.py`** — Phase 4a readable handoff package, `pm-share --package` (`scripts/pm_share.py`; merged from the former `scripts/pm_handoff.py` — the `pm-handoff` name is now reserved for a future external-tracker/design export, see `docs/plans/pm-os-modes-delivery-and-handoff-plan.md` Part C)
- `test_package_generates_per_story_files_with_traceability` — approving 01/02/03/06 then running `pm_share.py --package` assembles Product Epic indexes plus per-story files in the boss house-format by walking EPIC-### → US-### → FR-### → UJ-### → covering TC-###; the story lists both covering test cases, carries the authored story body and declared epic, and is stamped with source provenance + a "DO NOT EDIT HERE" banner.
