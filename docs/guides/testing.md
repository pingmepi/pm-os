...
- `test_prototype_screens_carry_scr_anchor_when_declared` — the lo-fi fallback renderer (`render_prototype_mockup`, used by `post-approve.py` only when the pm-prototype-html skill didn't already produce the mockup) extracts each screen's `SCR-###` id from "Screens to Include" onto the rendered `<article>` so the fallback is deep-linkable at `#SCR-###` too, matching the anchor requirement enforced on the interactive prototype (backlog #29); a screen named without an id carries an empty `screen_id`, not a bogus one.
- `test_color_swatches_wraps_hex_and_rgb_literals` / `test_color_swatches_ignores_non_color_hash_tokens` — color literals (`#RGB`/`#RRGGBB`/`rgb()`/`rgba()`) each gain a preview swatch styled with that color, so companions render colors not bare hashcodes (GH #62); `#`-prefixed non-colors like `#SCR-001` are left alone.

**`test_delivery_map.py`** — entity-title extraction (`lib/delivery_map.py`)
- `test_normalize_title_drops_dangling_bold_marker` / `test_normalize_title_preserves_snake_case_and_ampersands` — a mid-string dangling `**` (left when a bold-wrapped decl's opening marker is consumed with the id) is cleared, while snake_case and `&` survive (PMOS-011).
- `test_title_of_epic_strips_bold_wrapped_declaration` / `test_title_of_falls_back_to_id_when_titleless` — `title_of` on a bold-wrapped `EPIC-###` decl yields plain text with no unbalanced markup (the shape that had propagated to handoff filenames/headings and the Jira plan); a titleless decl falls back to the id.
...
- **Contract v3 — GenAI model availability/fallback (stages 03/08):** `test_genai_prd_model_rationale_without_availability_or_fallback_warns` (a GenAI PRD whose `Model Selection Rationale` names neither a deployment/availability path nor a fallback model gets `MODEL_SELECTION_INCOMPLETE` — WARNING, never blocking); `test_genai_prd_model_rationale_with_availability_and_fallback_is_clean` (naming the deployment path, region/quota and a named fallback with its switch trigger satisfies it); `test_non_genai_project_skips_the_model_selection_check` (`genai_flag: false` projects never see the check); `test_trd_model_serving_check_is_warning_only_and_version_exempt` (legacy TRDs without contract frontmatter stay exempt from `CONTRACT_VERSION_MISSING`, while current Stage 08 generations also run the v6/v7 section-shape contract below); `test_missing_genai_sections_are_not_flagged` (an absent section is not a finding — the GenAI sections are appended conditionally).
- **Contract v4 — Product Epics (`EPIC-###`):** `test_prd_product_epics_validate_declared_ownership` (a PRD with multiple declared Product Epics and one valid `Epic:` ref on every `US`/`FR` has no errors); `test_prd_product_epics_reject_missing_unknown_and_multiple_refs` (once Product Epics exist, missing, unknown, or multiple story/requirement epic refs are errors). A legacy PRD with no `## Product Epics` gets a warning only, so existing approved projects remain approvable.
- **Contract v5 — prioritization method + values:** `test_prd_prioritization_method_and_values_are_warning_only` verifies missing `## Prioritization Method` and missing labeled `Priority:` values on `US/FR/REQ` warn without blocking existing PRDs, while complete labeled values are clean for those checks.
- **Contract v6 — TRD required-section shape:** `test_trd_section_contract_is_warning_only_for_v6` verifies legacy TRDs without v6 frontmatter stay quiet, incomplete v6 TRDs emit warning-only `TRD_REQUIRED_SECTION_*` findings, and a complete v6 TRD is clean for section shape.
- **Contract v7 — labeled fields over vocabulary cues:** `test_qa_plan_test_case_labeled_fields_warn`, `test_screen_without_purpose_warns`, and `test_trd_task_labeled_fields_warn_for_v7` verify that TC/SCR/TSK blocks expose structural labels for the meaning downstream tools rely on, warning without blocking when the labels are missing.
- **Stage-04 PRD/design consistency:** `test_design_warns_when_prd_empty_invalid_behavior_is_missing` verifies PRD empty/invalid-input edge behavior must be reconciled in the design spec; `test_design_warns_on_placeholder_without_discoverability_affordance` verifies placeholder-based inputs name a visible label/helper/hint/description/ARIA affordance when accessibility says inputs are not placeholder-only.
- **Stage-05 prototype slice auditability:** `test_prd_journey_prototype_priority_warns_when_missing` verifies PRD journeys expose a declared prototype priority / validation-risk signal; `test_prototype_brief_warns_when_high_priority_journey_lacks_slice_decision` verifies high-priority journeys must be explicitly included or excluded in `What to Prototype`.
- **Generated history lineage:** `test_pm_snapshot_stamps_hash_and_copies_exact_artifact` verifies `pm_snapshot.py` stamps/verifies `generated_hash` and copies the exact artifact into `.history/`; `test_pm_check_warns_when_generated_snapshot_missing_or_mismatched` verifies `/pm-check` reports warning-only lineage degradation for missing or mismatched generated snapshots; `test_pm_check_accepts_matching_generated_snapshot` verifies a snapshot whose body hash *matches* its `generated_hash` yields **no** lineage warning — backlog #30, where `hash_artifact_body` was called unimported in `lib/consistency.py`, so every snapshot was falsely reported unreadable.
...
- **Traceability schema v5 / priorities + screens:** `test_index_is_schema_v5_with_priorities_tasks_epics_and_screens_maps` (the index declares v5 and carries PRD-declared priority, tasks, Product Epics, and screens maps); `test_rebuild_preserves_legacy_handoff_epic_tickets` (old synthetic `EPIC-01` ticket keys survive only as legacy metadata); `test_build_index_links_screens_and_requirements` (screens from an approved design spec's IA are recorded with what they serve, and each requirement is back-linked; a `UJ-###` stays a forward link only — journeys are not requirement entries); `test_screen_resolver_both_directions` (`screens_for_requirement`/`requirements_for_screen` resolve both ways case-insensitively, and a journey id resolves through the fallback scan); `test_unapproved_design_spec_contributes_no_screens` (only an **approved** stage 04 contributes, so unapproved design decisions never reach the handoff); `test_screen_outside_information_architecture_is_not_indexed`; `test_rebuild_preserves_screen_design_refs` (the reserved `design_refs` slot survives a rebuild); `test_missing_design_spec_yields_empty_screens`; `test_build_index_links_nfr_requirements` (backlog #32 — a TC covering `NFR-###` records those ids in its `requirements`, previously dropped).
...
- `test_enhancement_lifecycle.py` — captures hash-current approved baselines, preserves `project_type`/`entry_route`, makes same-ask start idempotent, refuses a second active ask, emits lifecycle telemetry without stage-like status/approval fields, reclaims a leftover lock whose owner process is gone, and still respects a lock whose owner is alive.
...
- `test_codebase_inventory.py` — inventories a lightweight monorepo, selects the ask-relevant slice, follows bounded inbound/outbound dependencies (including Python dotted `from .x` and JS/TS `./x` relative imports, so a non-ask-matched relative dependency lands in the impact cone, not non-touch), identifies non-touch paths and multi-surface impact, and lowers confidence for dynamic imports. Also accepts a `.zip` archive: it is extracted read-only to a throwaway temp tree and yields the same slice/surfaces as the extracted dir while the report names the archive (`test_scan_accepts_a_zip_archive_matching_the_directory_scan`), and a non-archive file errors clearly (`test_scan_rejects_a_non_archive_file`).
...
- `test_enhancement_root_cause_fixes.py` — the post-review root-cause regressions: a non-Git codebase recomputes to the same fallback fingerprint so `/pm-check` does not flag `ENHANCEMENT_REPOSITORY_DRIFT` (shared `lib/repo_fingerprint.py`, no `actual_sha is None` false-positive); a stage authored *during* the enhancement (absent from the frozen baseline) surfaces as all-`new` with heading/ordered-list/bare-line stable-id forms each yielding a fine-grained FR change (canonical `split_stable_id_blocks`, never a coarse `STAGE-03`); a removed baseline ref is recordable via `pm_handoff record` and never re-synthesized into a duplicate removal ticket on re-export; and a product stage in an enhancement is blocked in `pre-stage.py` until the affected-slice boundary is recorded. Second review round: a non-UI design stage that changes without SCR ids (coarse `STAGE-04` delta) ships its changed content in `--package` instead of the baseline note; and when stages 04/05 are unchanged the omitted prototype is never linked from a story or the screen map (no dangling `wireframes/prototype.html`).
...
- `test_approve_reconciles_pending_frontmatter_with_generation_evidence` — a generated stage whose frontmatter `status` drifted back to `pending` still approves; `pm_approve` reconciles from ground truth (non-empty body + a `stage_generated` event) rather than reporting a real artifact as ungenerated (GH #61).
- `test_approve_still_refuses_truly_ungenerated_stage` — the reconciliation doesn't over-reach: an empty-bodied slot with no generation event still fails with "generate it first".
...
- `test_pm_check_flags_downstream_approved_against_changed_upstream` — backlog #31 detector: an `approved` stage whose recorded `upstream_hashes_at_approval[uid]` no longer matches the still-`approved` upstream's current `content_hash` (the invalid state an interrupted stale-cascade leaves) is flagged `DOWNSTREAM_UPSTREAM_STALE`; a correctly-approved chain is clean.
...
- `test_plan_maps_jira_native_hierarchy` — `plan` maps to Jira's default hierarchy from declared Product Epics: `EPIC-###`→Epic, `US-###`→Story, `FR/REQ-###`→Task, approved single-parent `TSK-###`→Subtask; same-epic multi-ref tasks remain Tasks, cross-epic tasks stay unparented, and a `TSK-###` outside the Work Breakdown is excluded.
...
- **Offline CSV route:** `test_export_csv_writes_importer_file_and_guide` (`export` writes `jira-import.csv` + the import guide with one row per exportable item, declared epics before their children, `Parent Id` pointing at the owning epic's `Issue Id` when available, unparented cross-epic items parentless, and the stable id carried as both a column and a `pm-os-<id>` label); `test_export_csv_descriptions_are_jira_markup_with_provenance` (descriptions are converted to wiki markup — no residual `**`/backticks — and carry a provenance footer naming the source artifact); `test_export_csv_blocks_when_prd_not_approved` (`export` reuses the plan builder's approval gate and writes nothing).

**`test_share_package.py`** — Phase 4a readable handoff package, `pm-share --package` (`scripts/pm_share.py`, invoked via `/pm-handoff --package` — the `pm-share` skill was folded into `pm-handoff` 2026-07-28, `pm_share.py` unchanged as the mechanical engine). Since 2026-07-28 the package is split into `handoff/{dev,design,qa,business}/` audience folders (`AUDIENCE_CATEGORIES` in `pm_share.py`); `--audience` scopes a rebuild to one folder.
- `test_package_generates_per_story_files_with_traceability` — approving 01/02/03/06 then running `pm_share.py --package` assembles Product Epic indexes (`dev/epics/`, duplicated into `business/epics/`), decomposed requirement files (`dev/requirements/`, duplicated into `qa/requirements/`), and per-story files (`dev/stories/`, duplicated byte-identical into `qa/stories/`) in the boss house-format by walking EPIC-### → US-### → FR-###/REQ-### → UJ-### → covering TC-###. Each story carries the authored story body, declared epic context, full text of referenced requirements, and inline impact/NFR context so implementation-critical details survive even if `reference/` or `requirements/` folders are not copied. `jira-plan.json` (from a follow-up `pm_handoff.py plan`) lands at the `handoff/` root, never inside an audience folder.
- `test_package_story_inlines_ac_blocks_and_unknown_refs` — when a story references explicit `AC-###` blocks or an uncaptured FR/AC id, the package embeds the accepted criteria text where available and shows `— not captured in source —` for gaps instead of replacing context with hyperlinks.
- **Screens:** `test_package_maps_screens_to_each_story` (with an approved design spec, each `dev/stories/` file lists the `SCR-###` screens it touches — resolved through the spine via its requirements *and* journeys — with the screen's own body and the design spec added to the story's provenance stamps); `test_package_screen_map_reference_lists_coverage_both_ways` (`design/reference/screen-map.md`, duplicated into `qa/`, is the reverse view: each screen with what it serves, plus a "Stories with no screen" section naming uncovered stories, linked from `design/README.md`); `test_package_without_screen_ids_degrades_gracefully` (no approved design spec / no `SCR-###` ids → the not-captured marker and a screen map explaining what to add, never a crash or invention); `test_package_excludes_screens_from_an_unapproved_design_spec` (a stage 04 reopened as a draft contributes nothing to the story files or the screen map — unapproved design decisions must not ship as handoff material); `test_package_resolves_screens_from_a_stale_on_disk_index` (screens still resolve when `.traceability.yaml` predates the screens map, because the package builds the spine fresh rather than trusting the file). `test_screen_map_counts_journey_only_coverage_as_covered` (a story covered only through a journey link is not reported under "Stories with no screen" — the map derives coverage from the per-story resolution, not each screen's literal `serves` ids).
- **Screen links into the prototype (backlog #29):** `test_screen_link_emitted_when_anchor_present` (a prototype carrying `id="SCR-###"` anchors gets a working `../wireframes/prototype.html#SCR-###` link in both the story file and the screen map; the prototype copy is duplicated into `dev/wireframes/` too, not just `design/`/`qa/`, so the dev-folder link resolves); `test_screen_link_falls_back_when_anchor_absent` (a prototype that predates the anchor requirement still gets copied and linked to — a plain link, never a dead hash); `test_screen_link_omitted_without_a_prototype` (no `05-prototype-mockup.html` at all → screens still render, no link, no `wireframes/` folder). The validator half (`SCREEN_ANCHOR_MISSING`) lives in `tests/unit/test_artifact_contracts.py` (`test_html_missing_screen_anchor_is_flagged`, `test_html_with_all_screen_anchors_present_is_not_flagged`).
- **Backend work (TRD tasks per story):** `test_package_maps_backend_tasks_to_each_story` (with an approved TRD, each `dev/stories/` file's "Backend work (TRD tasks)" section lists the `TSK-###` tasks implementing the story's requirements — resolved through the fresh spine over its FRs — with each task's title + body and the TRD added to the provenance stamps and a `**Backend tasks:**` traceability line; a story implemented by no task shows the not-captured marker); `test_package_without_trd_degrades_backend_to_not_captured` (no approved TRD → the Backend work section and traceability line show `— not captured in source —`, never omitted or fabricated, and no TRD stamp enters provenance — the same graceful degradation as screens without a design spec).
...
- **v1.4.1 consistency fixes:** `test_story_frontmatter_is_valid_yaml_when_title_has_colon` (backlog #33 — a story title with a colon-space renders parseable YAML frontmatter, now serialized via `yaml.safe_dump`); `test_business_epic_has_no_dangling_story_links` (backlog #34 — business epics reference stories as plain text, not `../stories/` links that dangle where that audience has no stories folder, while dev epics keep working links that resolve); `test_journey_shared_screen_not_attributed_to_every_story` (backlog #36 — a screen naming a specific story is no longer pulled into every other story sharing its journey; direct-first resolution keeps the journey-only fallback); `test_global_ia_prose_does_not_leak_into_story_screen_body` (backlog #35 — trailing Information-Architecture prose not under its own heading no longer bleeds into a story's screen slice).
- `test_package_overview_and_reference_docs` — `business/00-overview.md` carries Who/What&Why/How from the brief+scope; `dev/reference/impact-analysis.md` and `nfrs.md` carry the PRD's Impact Analysis and NFR sections, duplicated into `qa/` too (so the dev/qa story files' own links to them never dangle).
- `test_package_projects_prd_trd_into_dev_and_design_spec_into_design` — dev gets the full PRD + TRD; design gets PRD, stories, NFRs, and the full approved design spec as read-only `reference/` projections (source-hash stamped), each scoped to its audience and wired into the README with a valid link (PMOS-002). Once the approved design spec exists, the draft-brief workflow file is not emitted.
- `test_package_design_audience_includes_draft_brief_and_workflow` — when stage 04 is a draft with `design_mode: brief`, the design package includes the approved PRD, story files, NFRs, `reference/design-brief.md`, and `DESIGNER-WORKFLOW.md`, clearly marking the brief as unapproved design input rather than a canonical spec.
- `test_design_ingest_preserves_brief_sections_and_package_emits_figma_state_links` — a returned `design-in/` package is ingested into stage 04 as `design_mode: ingested`, preserves PM-owned brief sections, emits stable `SCR-###` screens keyed to Figma node ids, and projects exact screen/state Figma links into matching dev/QA story dossiers without leaking unrelated screens.
- `test_package_projects_absent_optional_sources_as_notes` — when the TRD/design spec are not approved, their projections are still emitted (so README links never dangle) but carry an explicit "not approved" note instead of a body.
...
- `test_package_refuses_destructive_output_dir` — `--output .` (the project root/cwd) is rejected before any directory work, so a stray target can never erase `.meta.yaml`/approved artifacts (Codex PR #30/#31 P1 finding).
- `test_package_does_not_touch_unrelated_files_in_output_root` — since the audience-folder rework, the `handoff/` root itself is only ever `mkdir`'d, never wiped — pointing `--output` at an existing directory with unrelated content now succeeds (audience subfolders are created alongside it) rather than refusing outright, a deliberate behavior change from the old whole-directory guard.
- `test_package_refuses_existing_unmarked_audience_dir` — the destructive wipe-with-marker guard now lives one level deeper, at `handoff/<audience>/`: an existing non-empty audience folder lacking `.pm-os-handoff` is not deleted; only a prior package's audience folder (which carries the marker) is regenerated in place.
- `test_package_audience_scoped_rebuild_leaves_others_untouched` — `--audience dev` rebuilds only `handoff/dev/`; `design/`/`qa/`/`business/` (built by an earlier full run) stay byte-for-byte untouched even after the PRD changes and `dev/` picks up the change.
- `test_package_audience_conflict_stops_before_any_content_write` — if one of several requested audiences hits a marker conflict, no audience's *content* is written that run: folders already wiped before the conflict are left empty/marker-less (not falsely marked rebuilt) and folders after it in `dev/design/qa/business` order are never even created.
- `test_package_audience_flag_requires_package` — `--audience` without `--package` is a hard error, not a silently ignored flag.
...
**`test_handoff_share_interop.py`** — the `pm_handoff.py` ↔ `pm_share.py` collision the audience-folder rework was designed to close (previously untested): `test_jira_plan_then_package_does_not_collide` (Jira `plan` first, then `--package`, must both succeed with `jira-plan.*` untouched — this direction used to hard-fail before the rework); `test_package_then_jira_plan_does_not_collide` (the reverse order, which already worked, still does); `test_audience_scoped_package_survives_either_order` (a scoped `--audience dev` rebuild and a Jira `plan` never disturb each other, in either order, since `jira-plan.*` lives at the `handoff/` root and audience content lives one level down).
...
**Not covered by pytest — manual verification before shipping:** `skills/pm-handoff/SKILL.md`'s `$ARGUMENTS` prose-parsing across its six invocation shapes (bare/`jira`/`jira --offline`/`--package`/`--package --audience ...`/`--raw`), and the implicit Step-0 local-package refresh before the Jira flow (including `--no-package-refresh` and failing fast on an unapproved PRD) — these are agent-side prose behaviors, not exercised by a script-level test. Walk them once against a real scratch project through an actual Claude/Codex session before relying on them.
...
- context-import upstream guardrail: `commit … --status approved` blocks when an upstream stage is still pending, stopping the E3 prototype-as-input pathway from stamping an imported prototype into an approved stage 05 with no upstreams (`test_commit_approved_blocks_on_pending_upstream`, GH #59/#60); `--allow-missing-upstream` is the explicit escape and imported artifacts are stamped `fidelity: unverified` (`test_commit_imported_allow_missing_upstream_and_marks_fidelity`).
...
- prepare-codebase zip source (`test_prepare_codebase_zip.py`): a `.zip` extracts into `.codebase/`, flattens a single wrapper folder, and is git-init'd into a one-commit checkout so E2 can pin a real `codebase_ref` (`test_zip_extracts_flattens_and_becomes_a_git_checkout`); re-preparing the same archive reuses while a different archive refuses to overwrite the extraction/ref (`test_same_zip_reuses_and_different_zip_is_refused`); a `../` traversal entry is rejected before any write (`test_zip_slip_entry_is_rejected_without_writing_outside`); a non-archive file gets a clear error (`test_non_zip_file_is_rejected_clearly`).
...
| `test_pm_check_flags_downstream_approved_against_changed_upstream` (lives in `test_approval_and_staleness.py`) | `_check_downstream_upstream_hashes`: an `approved` stage whose recorded `upstream_hashes_at_approval[uid]` ≠ the still-`approved` upstream's current `content_hash` is `DOWNSTREAM_UPSTREAM_STALE` (backlog #31 — the invalid state an interrupted stale-cascade leaves; consumes the previously-unread field). |
