...
- `test_package_refuses_a_draft_prd` — a PRD that exists but is only `draft` (not approved/edited) is refused, so unreviewed product decisions can't be published as canonical handoff (Codex PR #31 finding).
