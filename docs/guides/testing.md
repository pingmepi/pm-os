...
- `git_sync_local` *(connection)*: approval pushes to a **local bare** feedback repo (real git path, `PM_OS_SYNC_BLOCKING=1` inline mode the fixture defaults to); the deferred default backgrounds the push so approval returns immediately yet the detached push still lands centrally (`test_deferred_approval_sync_does_not_block`, backlog #6); `pm_sync` backfills all projects; `--verify` reports chains intact.
