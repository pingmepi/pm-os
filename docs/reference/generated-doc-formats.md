...
| `handoff/{dev,design,qa,business}/` | `pm_share.py --package` (via `/pm-handoff --package [--audience ...]`) | Read-only projection of the approved pipeline, split per audience folder: each gets its own `README.md`; `00-overview.md` (business only); one `epics/EPIC-###-*.md` per declared Product Epic (dev, business); one `stories/US-###-*.md` per story with screens/tests/tasks; audience-specific `reference/`; and optional prototype. During an active E2 cycle it automatically contains only changed stories/requirements plus minimum ancestor closure, and the README records removals, affected surfaces, invariants, compatibility, rollout, rollback, and baseline provenance. `--audience` rebuilds one folder, leaving siblings untouched; `handoff/README.md` is the hub. Regenerated wholesale — never hand-edited. |
