...
| `handoff/{dev,design,qa,business}/` | `pm_share.py --package` (via `/pm-handoff --package [--audience ...]`) | Read-only projection of the approved pipeline, split per audience folder: each gets its own `README.md`; `00-overview.md` (business only); one `epics/EPIC-###-*.md` per declared Product Epic (dev, business); one `stories/US-###-*.md` per story with the `SCR-###` screens it touches (linked into the prototype), its covering QA scenarios, and — when the TRD is approved — the `TSK-###` backend tasks that implement it (dev, qa); `reference/` docs distributed per audience (user journeys in all four; prioritization and impact analysis in dev/business/qa; NFRs in dev/qa; QA scenarios in qa; **screen map** in design/qa); `wireframes/prototype.html` in dev/design/qa. `--audience` rebuilds one folder, leaving siblings untouched; a lightweight `handoff/README.md` hub (not itself an audience folder) always lists which ones currently exist. Regenerated wholesale per audience folder — never hand-edited. |
