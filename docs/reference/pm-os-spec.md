...
**Status:** ✅ **Kernel implemented as of v0.4.8** — the core stages (01–07) plus the optional TRD (08) and roadmap (09) capstones, approval gates (`hooks/pre-stage.py`, `hooks/post-approve.py`), the status/hash state machine, config, telemetry/feedback logs, HTML companions, and cross-runtime install (Claude + Codex) are all live. The shipped system has since grown well beyond this build spec (traceability spine, `/pm-check`, context overlay + adaptive pack, enhancement mode, Jira handoff, and more) — this document is the **original build spec**, not a live state record; for what exists today see `docs/roadmap/current-state-review.md`, `CHANGELOG.md`, and the as-built `ARCHITECTURE.md`. ⚠️ Parts of this spec are **aspirational and were never built** — `lib/edit_distance.py` and `lib/embeddings.py` (edit distance actually lives in `lib/text_metrics.py`; embedding-based semantic distance was never built), the `hooks/post-tool-use.py` and `hooks/session-end.py` hooks (§9.3/§9.4), the `session_end` telemetry event, and the "Sonnet default; Opus for 03/06" line in §6 step 9 (superseded by the runtime-neutral tier policy in §7). When this spec and the code disagree, trust the code and `ARCHITECTURE.md`.
...
artifact_contract_version: 7   # present on contract-validated Stage 03–06 and new Stage 08 generations
...
- **Model selection:**
  ...
  - Shared config stores `deep_reasoning_stages: ["00c", "00w", "00u", "03", "04", "06", "08", "09"]`.
