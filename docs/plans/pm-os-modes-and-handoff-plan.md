...
**Status:** 🟡 **Part A implemented (v0.5.9 / v0.6.0); Part B not implemented.** Enhancement mode shipped: `--mode enhancement`, `--codebase <url-or-path>`, `project_type`/`codebase_path`/`codebase_ref` in `.meta.yaml` (schema v3), conditional `00c` codebase-understanding stage, `prepare-codebase` subcommand in `pm_context_import.py`, codebase drift signal in `pm_status.py`. Part B (engineering handoff to Jira/Linear/Figma via MCP) remains unbuilt — tracked as Phase 4 in `docs/roadmap/current-state-review.md` §7.
>
> **⚠️ Naming note (2026-07-15, unresolved — needs a PM decision):** this plan reserved the command name **`/pm-handoff <target>`** (`jira`/`linear`/`figma`) for Part B (see §8 below). A *different* capability — a local, human-readable handoff-package generator (per-story files decomposed from the approved PRD, closer in spirit to Phase 4a's "local handoff packet") — has since shipped under `skills/pm-handoff/` using the **same base command name with no sub-command**, via [PR #30](https://github.com/pingmepi/pm-os/pull/30). The two were never reconciled against each other. A non-breaking path exists — treat bare `/pm-handoff` (no argument) as the already-shipped local package, and reserve `/pm-handoff <jira|linear|figma>` for this plan's Part B when it's eventually built — but that's a naming call for Karan to make, not something resolved unilaterally here.
