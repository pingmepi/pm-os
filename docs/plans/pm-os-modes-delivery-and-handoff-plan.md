...
**Status:** 🟡 **Part A's full E2 enhancement pathway shipped in v1.5.0 (PR #68); Part B partially built; Part C partly shipped (Jira; Linear/Figma unbuilt).** E2 adds same-project baseline cycles, a deterministic read-only inventory/affected-slice/impact-cone scan, decision boundary, multi-surface stage overlays 01–09, non-UI validation behavior, delta/consistency/status gates, delta-only package/Jira export, explicit refresh, completion, and two-cycle provenance. The pre-existing enhancement scaffold remains the first-intake route. **Part B — the delivery model (scope tiers + delivery increments) — is designed here (2026-07-27); B0 plus prerequisite priority/TRD contract work are shipped, while tiers and increments remain unbuilt.** Part C (external engineering handoff) is partly shipped: `/pm-handoff jira` — both the Atlassian-MCP create route and the `--offline` CSV export — landed v1.2.0 (screen mapping v1.3.0); `/pm-handoff linear`, Figma pull/push, and design-token→React codegen remain unbuilt. Delivery-model and unbuilt-handoff work is tracked as Phase 4 in `docs/roadmap/current-state-review.md` §7 and as backlog #28.
>
...
- **`/pm-handoff figma` — 🔴 unbuilt.** Two directions:
  ...
  - **Direction warning — read before spending this name.** Both bullets above are **outbound/PM-initiated**: the PM reaches into Figma. A separate design (`pm-os-external-design-authority-plan.md`, recorded 2026-08-13) covers the **inbound** case, where a *designer* authors the IA + Figma and it enters stage 04 as a gated upstream input carrying decision authority. That is a different capability, not this one's other half, and its §5 design deliberately mints **no** new command name (it re-runs `/pm-stage-04-design-spec` against a `design-in/` directory). Do not assume `figma` covers both.
