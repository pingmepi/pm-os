...
| Entry pathways + interview primitive | Pathway 2 (prototype entry) + interview shipped v1.4.3; **standalone `/pm-interview` re-run + known-unknowns surfacing (E3) shipped v1.4.5**; Pathway 3 **E2 implemented on `feat/e2-affected-slice-enhancement`, pending final review/merge: same-project frozen baselines, read-only `00c` inventory/slice/impact cone, decision boundary, multi-surface stages 01–09, consistency/status, delta-only package/Jira handoff, explicit refresh/completion, and two-cycle provenance** | `../plans/pm-os-entry-pathways-plan.md` E2.0–E2.7; `../plans/pm-os-e2-execution-runbook.md` Loops 0–12 |
...
`/pm-new --entry enhancement --codebase <url-or-path>` remains the one-time external-product intake. On the E2 branch, `/pm-enhance` then owns every same-project cycle without changing the product's first-intake identity: it freezes approved artifacts and repository evidence, drives the `00c` inventory/affected-slice/impact-cone scan and decision boundary, conditions stages 01–09, derives checked stable-ID deltas, scopes package/Jira output, supports explicit clean-ref refresh with staleness, and completes the cycle so the canonical artifacts become the next baseline. `/pm-status` derives active/completed guidance; `/pm-check` blocks context/baseline/ref drift and traceability gaps. The target codebase remains read-only. See `docs/plans/pm-os-modes-delivery-and-handoff-plan.md` for Part B (tiers/increments, still separate) and Part C (Jira shipped; Linear/Figma unbuilt).
