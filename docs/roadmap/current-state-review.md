...
| Screen→story mapping (`SCR-###`) | Implemented (v1.3.0) | Stage-04 Information Architecture declares stable `SCR-###` screens with a `Serves:` trace; `.traceability.yaml` (schema v4, screens added in v3) indexes a `screens:` map (approved stage 04 only); `/pm-share --package` renders "Screens this story touches" + `reference/screen-map.md`; `/pm-check` validates unique ids, real-id traces, and per-story coverage |
| Product Epic ownership (`EPIC-###`) | Implemented (B0 follow-up) | Stage-03 Product Epics declare real product workstreams/outcomes; every `US-###` and `FR/REQ-###` cites exactly one declared `Epic:` once the section exists. `.traceability.yaml` schema v4 indexes `epics:` membership and preserves old synthetic `EPIC-01` refs as legacy metadata only. |
| GenAI model-selection/serving checks (contract v3) | Implemented (v1.3.0) | `lib/artifact_contracts.py` `CONTRACT_VERSION=4` (v3 checks preserved): stage-03 `MODEL_SELECTION_INCOMPLETE` and stage-08 `MODEL_SERVING_INCOMPLETE` — both WARNING-only and fired only when `genai_flag: true`, so non-GenAI projects and older artifacts are untouched |
