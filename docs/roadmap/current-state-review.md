...
| Jira handoff export (Phase 4b) | Implemented (v1.2.0; offline route v1.3.0) | `/pm-handoff jira` (`skills/pm-handoff/` + `scripts/pm_handoff.py`): parses the approved PRD (+ TRD) into a `US→epic / FR→story / TSK→task` map, dry-run → confirm → create via the Atlassian MCP → `record` writes ticket keys back into `.traceability.yaml`. `--offline` (`lib/jira_markup.py`) writes `handoff/jira-import.csv` for Jira's own importer — no connector, no tokens |
| Screen→story mapping (`SCR-###`) | Implemented (v1.3.0) | Stage-04 Information Architecture declares stable `SCR-###` screens with a `Serves:` trace; `.traceability.yaml` (schema v3) indexes a `screens:` map (approved stage 04 only); `/pm-share --package` renders "Screens this story touches" + `reference/screen-map.md`; `/pm-check` validates unique ids, real-id traces, and per-story coverage |
| GenAI model-selection/serving checks (contract v3) | Implemented (v1.3.0) | `lib/artifact_contracts.py` `CONTRACT_VERSION=3`: stage-03 `MODEL_SELECTION_INCOMPLETE` and stage-08 `MODEL_SERVING_INCOMPLETE` — both WARNING-only and fired only when `genai_flag: true`, so non-GenAI projects and v1/v2 artifacts are untouched |
