...
**Date:** 2026-06-12 · **Updated:** 2026-08-05 (v1.5.0; see CHANGELOG for full history)

> **Version summary:** Phase 1 runtime parity and Phase 2 flexible intake shipped in v0.4–v0.5.2; context overlay + telemetry metrics in v0.5.3; deep-reasoning tier expansion in v0.5.4; automated test suite in v0.5.5; telemetry/gate hardening in v0.5.6–v0.5.7; richer context intake in v0.5.8; **enhancement mode + `00c` codebase-understanding + offline install** in v0.5.9–v0.5.10; `pm-prototype-html` skill in v0.5.11; **artifact quality contracts (Stages 03–05) + research-safe prototypes** in v0.6.0; **Phase 3.5 traceability spine** (stable `REQ`/`TC` IDs, `.traceability.yaml` resolver) and **adaptive context pack** (composite hashing, schema v4, modular pack across stages 01–09) in v1.0.5–v1.0.6; **`/pm-check` consistency toolkit** in v1.0.7; **PRD contract v2 + readable handoff package (`/pm-share --package`)** in v1.0.10–v1.0.12; **reliability + traceability hardening** (unified `TC` extractors, `--reapprove`, runtime-version telemetry) in v1.0.14; **deferred non-blocking central sync + portable cache lock** (#6) in v1.1.0; **Phase 3.5b TRD task IDs (`TSK-###`, traceability schema v2) + Phase 4b Jira handoff export (`/pm-handoff`)** in v1.2.0; **`SCR-###` screen→story mapping (traceability schema v3), offline Jira CSV export (`/pm-handoff jira --offline`, `lib/jira_markup.py`), and GenAI model-selection/serving checks (artifact contract v3)** in v1.3.0; **screen-map journey-coverage fix** in v1.3.2; **Product Epics (`EPIC-###`, artifact contract v4, traceability schema v4) aligning readable and Jira handoff** in the B0 follow-up; **Part 1 integrity stack** in v1.3.6–v1.3.11: PRD priorities, Stage 08 required-section validation, labeled-field contracts, Stage 04 input reconciliation, Stage 05 slice auditability, deterministic generated snapshots, artifact contract v7, and traceability schema v5; **v1.4.1 consistency-defect cluster** (backlog #30–#36 — snapshot-import bug, NFR traceability, story-frontmatter YAML, handoff leak fixes, plus the `DOWNSTREAM_UPSTREAM_STALE` detector) in v1.4.1–v1.4.3, and the **prototype entry pathway + interview primitive** (E0–E1) merged in v1.4.3; **plans/roadmap reconciliation** in v1.4.4; **standalone `/pm-interview` re-run (E3) + decks/sheets read via the `pptx`/`xlsx` skills instead of being pre-condemned lossy** in v1.4.5; **E3 import upstream-presence guardrail (imported artifacts stamped `fidelity: unverified`), `pm-approve` reconciliation of a drifted `pending` status against generation evidence, and color-token swatches in the HTML companions** (GH #59–62) in v1.4.6; and **full PRD/TRD/design-spec read-only projections in the audience handoff packages, entity-title Markdown normalization (dangling `**` no longer leaks into filenames/headings/Jira), and an actionable offline-install `pm_os_update` message** (GH #63 — PMOS-002/011) in v1.4.7; and the **E2 affected-slice enhancement pathway** (`/pm-enhance` — same-project frozen baselines, `00c` inventory/slice/impact cone, decision boundary gating stages 01–09, `.zip` codebase sources, delta-only package/Jira handoff, refresh/completion; PR #68) in v1.5.0. §2/§3 below reflect that state. **Current `.meta.yaml` schema is v5** (`lib/project.py`; v5 added `entry_route`); `.traceability.yaml` schema is v5 (config.yaml schema is a separate v1).
...
The current codebase (v1.5.0) is a strong **local-first product-definition MVP** with flexible intake, enhancement mode (now the full E2 same-project affected-slice cycle), a pluggable company/team context overlay, deterministic artifact quality contracts, a requirements-to-test-case-to-task-to-screen traceability spine, a read-only consistency check (`/pm-check`), a decomposed per-audience handoff package (`/pm-handoff --package`), and a Jira export (`/pm-handoff jira`, connector or offline CSV) — all through the single `/pm-handoff` skill (`/pm-share` was folded into it).

It can scaffold a project from a business statement **or from existing PM-authored context** (research, brief, scope, PRD, design notes) via `/pm-context-import`; generate staged product artifacts; require human approval between stages; track status/hashes/origin in local files; check project consistency (`/pm-check`); record telemetry/feedback; and export approved artifacts — as raw text or a decomposed per-audience handoff package (`/pm-handoff --package [--audience dev|design|qa|business]`).
...
| Share/export | Implemented, basic | `scripts/pm_share.py` (invoked via `/pm-handoff --raw`/`--package`) exports approved/edited artifacts |
...
| Schema versioning + migration | Implemented | `.meta.yaml` `schema_version: 5`; `lib/project.migrate_meta` upgrades older projects in place (v2 added `00` stage + per-stage `origin`; v3 added `project_type`/`codebase_path`/`codebase_ref`; v4 added the composite-hash adaptive context pack; v5 added `entry_route`) without disturbing approvals |
...
| Enhancement mode | Implemented (v0.5.9) | `/pm-new --entry enhancement --codebase <source>` scaffolds a project with `project_type: enhancement`, `codebase_path`, `codebase_ref` (schema v3). `pm_context_import.py prepare-codebase` clones (git URL), validates (local dir), or extracts (`.zip`, git-init'd) the codebase into read-only `.codebase/` and records its git SHA. `/pm-status` shows mode and warns on codebase drift. Stage-01 brief in enhancement mode frames the request as a delta against the existing product. Product stages 01–09 are gated on a recorded affected-slice boundary (`pre-stage.py`) |
...
| Artifact quality contracts (Stages 03–06, 08) | Implemented (v0.6.0; expanded through v1.3.11) | `lib/artifact_contracts.py` (`CONTRACT_VERSION=7`) + `scripts/pm_validate_artifact.py`. PRDs require `UJ-###` journeys, Product Epics, priority/prototype-priority labels, and labeled story fields; design specs require journey-to-flow traceability, interaction model, screen traces, and input behavior reconciliation; prototype briefs require audience/mode separation, validation plan, and high-priority journey include/exclude decisions; QA plans and TRDs use labeled-field checks. Strict mode blocks generation on blocking findings; warn mode at approval/import logs `artifact_validation_warning` telemetry and continues |
...
| Readable handoff package (`/pm-handoff --package`) | Implemented (v1.0.10–1.0.12; audience-scoped folders + `/pm-share` fold-in 2026-07-28; full PRD/TRD/design-spec projections v1.4.7) | `scripts/pm_share.py --package` generates a decomposed handoff package (per-story files + reference docs) from the approved pipeline, split into `handoff/{dev,design,qa,business}/` (`--audience` rebuilds one folder, leaving siblings untouched). Each audience also carries the **full source-of-truth documents** behind the decomposition as read-only, source-hash-stamped `reference/` projections: `prd.md` + `trd.md` in dev, `design-spec.md` in design (an unapproved optional source projects an explicit note instead of a body, so links never dangle). Originally shipped as `/pm-share --package`; that skill has since been folded into `/pm-handoff`, which is now the sole callable export skill (raw text, package, and Jira). Requires an *approved* (not `edited`) PRD and guards against overwriting existing output. |
...
| TRD task IDs (Phase 3.5b) | Implemented (v1.2.0) | Stage-08 TRD contract requires a **Work Breakdown** enumerating `TSK-###` tasks, each `Implements:` a PRD requirement; `.traceability.yaml` (schema v2) indexes a `tasks:` map with reverse requirement↔task links; resolver queries + `pm_trace.py task`; `/pm-check` validates unique/sequential ids and coverage |
| Jira handoff export (Phase 4b) | Implemented (v1.2.0; offline route v1.3.0; Product Epics B0 follow-up) | `/pm-handoff jira` (`skills/pm-handoff/` + `scripts/pm_handoff.py`): parses the approved PRD (+ TRD) into a Jira-native `EPIC→Story/Task→Subtask` map from declared `EPIC-###` ownership, dry-run → confirm → create via the Atlassian MCP → `record` writes ticket keys back into `.traceability.yaml`. `--offline` (`lib/jira_markup.py`) writes `handoff/jira-import.csv` for Jira's own importer — no connector, no tokens |
| Screen→story mapping (`SCR-###`) | Implemented (v1.3.0; direct prototype links backlog #29) | Stage-04 Information Architecture declares stable `SCR-###` screens with a `Serves:` trace; `.traceability.yaml` (current schema v5; screens added in v3) indexes a `screens:` map (approved stage 04 only); `/pm-handoff --package` renders "Screens this story touches" + `reference/screen-map.md`, each screen linked into the copied prototype (`../wireframes/prototype.html#SCR-###`) when the prototype carries a matching `id="SCR-###"` anchor — `pm-prototype-html` (`prompt_version >= 0.3.0`) requires the anchor + a hash-router, and `validate_prototype_html` flags a missing one; `/pm-check` validates unique ids, real-id traces, and per-story coverage |
| Product Epic ownership (`EPIC-###`) | Implemented (B0 follow-up) | Stage-03 Product Epics declare real product workstreams/outcomes; every `US-###` and `FR/REQ-###` cites exactly one declared `Epic:` once the section exists. `.traceability.yaml` current schema v5 (epics added in v4) indexes `epics:` membership and preserves old synthetic `EPIC-01` refs as legacy metadata only. |
| GenAI model-selection/serving checks (contract v3) | Implemented (v1.3.0; preserved in contract v7) | Stage-03 `MODEL_SELECTION_INCOMPLETE` and stage-08 `MODEL_SERVING_INCOMPLETE` are both WARNING-only and fired only when `genai_flag: true`, so non-GenAI projects and older artifacts are untouched |
| Prioritization + prototype slice auditability | Implemented (v1.3.6, v1.3.10) | Stage 03 adds `## Prioritization Method`, labeled `Priority:` values on `US/FR/REQ`, and `Prototype priority:` on `UJ-###`; `.traceability.yaml` schema v5 indexes priority values; stage 05 warns when high-priority journeys lack explicit include/exclude decisions. |
| TRD required-section validation | Implemented (v1.3.7) | Stage 08 now has a warning-only required-section contract for new TRDs, while legacy/imported TRDs remain non-blocked. |
| Labeled-field hardening | Implemented (v1.3.8) | Contract checks use labeled fields for the high-risk declaration types that previously depended on cue matching: stage-03 journeys/stories, stage-04 screens, stage-06 test cases, and stage-08 tasks. |
| Deterministic generated history | Implemented (v1.3.11) | Stage skills call `scripts/pm_snapshot.py` after writing drafts; `/pm-check` warns when generated snapshots are missing or `generated_hash` mismatches. |
...
| Entry pathways + interview primitive | Pathway 2 (prototype entry) + interview shipped v1.4.3; **standalone `/pm-interview` re-run + known-unknowns surfacing (E3) shipped v1.4.5**; Pathway 3 **E2 shipped v1.5.0 (PR #68): same-project frozen baselines, read-only `00c` inventory/slice/impact cone, decision boundary, multi-surface stages 01–09, the mandatory affected-slice gate on stages 01–09, `.zip` codebase sources, consistency/status, delta-only package/Jira handoff, explicit refresh/completion, and two-cycle provenance** | `../plans/pm-os-entry-pathways-plan.md` E2.0–E2.7; `../archive/pm-os-e2-execution-runbook.md` Loops 0–12 |
...
Shipped 2026-06-17 as `/pm-context-import` (see `../archive/pm-os-ingest-plan.md` §0). The realized design is broader than the original per-stage import below: the PM provides **all the context they have**; PM-OS builds a **gated context wiki** (`00-context-wiki.md`) + **gated understanding doc** (`00-context-understanding.md`), then adopts the artifacts the PM authored (`origin: imported`) and faithfully backfills the upstream gaps below them (`origin: backfilled`), governed by a feasibility map. The business statement became a normal gated stage (`00`), and all present stage-00 docs must be approved before stage 01 (the gate lists exactly what's pending). `.meta.yaml` moved to `schema_version: 2` at Phase 2 (adding the `00` stage + per-stage `origin`; **now v5** — see the migration history above) with an in-place `migrate_meta` so existing projects keep working. Codebase-understanding has since been unified into the same stage-00 framework: enhancement projects (`/pm-new --mode enhancement --codebase <url-or-path>`, which brought `schema_version: 3`) get a conditional gated `00c` codebase-understanding doc, produced by `/pm-context-import` via a read-only codebase scan, alongside the wiki/understanding docs. `.docx`/`.pdf` conversion is resolved (registered as document types in `pm_context_import.py`, extended to images/PPTX/XLSX by the adaptive-context-pack's Phase 1). Remaining: dogfooding a real Indegene enhancement end-to-end (I4).
...
`/pm-new --entry enhancement --codebase <source>` (git URL, local directory, or `.zip` archive) remains the one-time external-product intake. Shipped v1.5.0 (PR #68), `/pm-enhance` then owns every same-project cycle without changing the product's first-intake identity: it freezes approved artifacts and repository evidence, drives the `00c` inventory/affected-slice/impact-cone scan and decision boundary, conditions stages 01–09, derives checked stable-ID deltas, scopes package/Jira output, supports explicit clean-ref refresh with staleness, and completes the cycle so the canonical artifacts become the next baseline. `/pm-status` derives active/completed guidance; `/pm-check` blocks context/baseline/ref drift and traceability gaps. The target codebase remains read-only. See `docs/plans/pm-os-modes-delivery-and-handoff-plan.md` for Part B (tiers/increments, still separate) and Part C (Jira shipped; Linear/Figma unbuilt).
...
- **Audience-scoped folders + direct prototype links + `/pm-share` fold-in (2026-07-28, dev-lead feedback):** the package now splits into `handoff/{dev,design,qa,business}/`, each audience getting only what's relevant to it (`--audience` rebuilds one folder, leaving siblings untouched); every screen mention now links directly into the interactive prototype, gated by a new anchor requirement in `pm-prototype-html` and a matching `validate_prototype_html` check; and the `/pm-share` skill itself was retired, its raw/package modes folded into `/pm-handoff`, which is now the single callable export skill.
...
Shipped as `/pm-handoff jira` (`skills/pm-handoff/` + `scripts/pm_handoff.py`), gated on approved status and built on the Phase 3.5/3.5b stable IDs plus Stage 03 Product Epics. The tracker is **Jira** (the single tracker chosen per the roadmap principle — not both). `pm_handoff.py plan` parses the approved PRD (+ approved TRD) into a tracker-agnostic ticket map — declared `EPIC-###` → Epic, `US-###` → Story under its declared epic, `FR-###`/`REQ-###` → Task under its declared epic, and approved `TSK-###` → Subtask when it implements exactly one exported item (same-epic multi-ref tasks become Tasks; cross-epic/unresolved tasks stay unparented) — and writes a PM-readable dry-run (`handoff/jira-plan.md`) plus a machine map (`.json`), fully offline. The skill then runs **dry-run → PM confirms → create via the Atlassian MCP → `pm_handoff.py record`**, which writes each returned ticket key into the matching epic/requirement/task `tickets: []` slot in `.traceability.yaml` and logs a `handoff_exported` telemetry event. **Offline route (v1.3.0, tech-lead feedback):** `/pm-handoff jira --offline` (`pm_handoff.py export`) renders the same plan as `handoff/jira-import.csv` + an import guide, with descriptions converted to Jira wiki markup (`lib/jira_markup.py`) and `Issue Id`/`Parent Id` parent linking — no connector, no tokens; the PM runs Jira's own CSV importer and records the keys back with the same `record` step. GitHub/GitLab PR/commit refs and Figma source links remain **separate, later, read-only "reference capture" units** — not bundled here.
...
> **Historical sketch (2026-06-12).** This section captures the *original* data-model proposal; the
> `schema_version: 2` in the YAML below was the first version marker proposed, not the current shape.
> **As shipped, `.meta.yaml` is now `schema_version: 5`** (`lib/project.py`), and `.traceability.yaml`
> is a separate `schema_version: 5`. Read the numbers below as the proposal's starting point.
...
For fine-grained build sequencing, each plan carries its own phase table (`../plans/pm-os-entry-pathways-plan.md`, `../plans/pm-os-consistency-spine-plan.md`, `../plans/pm-os-modes-delivery-and-handoff-plan.md`); caught defects/gaps live in `backlog.md`. (The earlier one-time cross-cutting consolidation is archived at `../archive/development-order.md`.) The coarse phase order below stays for historical continuity.
...
4. ~~Add `schema_version` and the migration path so existing projects survive new fields.~~ (done — `.meta.yaml` has since advanced to `schema_version: 5` via in-place `migrate_meta`)
