...
The current codebase (v1.3.11) is a strong **local-first product-definition MVP** with flexible intake, enhancement mode, a pluggable company/team context overlay, deterministic artifact quality contracts, a requirements-to-test-case-to-task-to-screen traceability spine, a read-only consistency check (`/pm-check`), a decomposed per-story handoff package (`/pm-share --package`), and a Jira export (`/pm-handoff jira`, connector or offline CSV).
...
| Screen→story mapping (`SCR-###`) | Implemented (v1.3.0) | Stage-04 Information Architecture declares stable `SCR-###` screens with a `Serves:` trace; `.traceability.yaml` (current schema v5; screens added in v3) indexes a `screens:` map (approved stage 04 only); `/pm-share --package` renders "Screens this story touches" + `reference/screen-map.md`; `/pm-check` validates unique ids, real-id traces, and per-story coverage |
