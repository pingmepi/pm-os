...
| Readable handoff package (`/pm-handoff --package`) | Implemented (v1.0.10–1.0.12; audience-scoped folders + `/pm-share` fold-in 2026-07-28) | `scripts/pm_share.py --package` generates a decomposed handoff package (per-story files + reference docs) from the approved pipeline, split into `handoff/{dev,design,qa,business}/` (`--audience` rebuilds one folder, leaving siblings untouched). Originally shipped as `/pm-share --package`; that skill has since been folded into `/pm-handoff`, which is now the sole callable export skill (raw text, package, and Jira). Requires an *approved* (not `edited`) PRD and guards against overwriting existing output. |
...
| Entry pathways + interview primitive | Pathway 2 (prototype entry) + interview shipped v1.4.3; **standalone `/pm-interview` re-run + known-unknowns surfacing (E3) built, pending PM review** (unreleased); Pathway 3 **E2 implemented on `feat/e2-affected-slice-enhancement`, pending final review/merge: same-project frozen baselines, read-only `00c` inventory/slice/impact cone, decision boundary, multi-surface stages 01–09, consistency/status, delta-only package/Jira handoff, explicit refresh/completion, and two-cycle provenance** | `../plans/pm-os-entry-pathways-plan.md` E2.0–E2.7; `../plans/pm-os-e2-execution-runbook.md` Loops 0–12 |
