...
> **As shipped, `.meta.yaml` is now `schema_version: 4`** (`lib/project.py`), and `.traceability.yaml`
...
4. ~~Add `schema_version` and the migration path so existing projects survive new fields.~~ (done — `.meta.yaml` has since advanced to `schema_version: 4` via in-place `migrate_meta`)
