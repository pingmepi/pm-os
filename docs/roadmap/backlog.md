...
**Decision (this conversation, 2026-07-28):** generate one bulk interactive prototype only (already built) rather than a separate HTML file per screen — parallel per-screen generation would have the LLM author the same UI twice (once in `05-prototype-mockup.html`, once per screen file), which drifts across regenerations with no mechanical guarantee they'd match. Screenshots were also considered (headless capture via Playwright) and explicitly declined for now — no new runtime dependency — in favor of a links-only approach.

**Fixed (this change):**
- `skills/pm-prototype-html/SKILL.md` (`prompt_version: 0.3.0`) — every top-level screen/destination container must carry `id="SCR-XXX"` matching the design spec's screen ids, plus a hash-router (`location.hash`) that jumps straight to that screen on load, in participant mode.
- `lib/artifact_contracts.py` `validate_prototype_html` — new `SCREEN_ANCHOR_MISSING` ERROR check: every `SCR-###` the approved design spec declares must have a matching `id="SCR-XXX"` in the HTML, so a story's screen link can never silently point at nothing.
- `templates/handoff-story.md.j2` + `scripts/pm_share.py` (`_screen_link`, `_screen_map_table`) — every screen mention (each story's "Screens this story touches" section, and `reference/screen-map.md`) now links into the copied prototype (`../wireframes/prototype.html#SCR-XXX`).
- **Backward-compat, as scoped:** `pm_share.py` reads the copied prototype once and checks for `id="SCR-XXX"` **per screen** before emitting a hash link — link only if the anchor is actually present, otherwise falls back to a plain (non-hash) prototype link, or omits the link entirely if no prototype exists (matching the existing `NOT_CAPTURED` convention). A project whose prototype predates the anchor requirement degrades safely rather than producing dead links; regenerating just the HTML (`/pm-prototype-html` standalone) upgrades it with no re-approval or staleness cascade.
- Landed alongside the audience-folder split: `wireframes/prototype.html` is now copied into `dev/`, `design/`, and `qa/` (not just design/qa) specifically so the dev-folder story links resolve.
- Tests: `SCREEN_ANCHOR_MISSING` validator coverage, and package-build tests for both the anchor-present and anchor-absent (pre-upgrade) cases.

**Evidence:** `lib/artifact_contracts.py:41-44` (`SCREEN_ID_RE`), `:505-557` (`split_screen_blocks`/`screen_serves`); `scripts/pm_share.py` (`_screen_link`, screen resolution into story context, `_screen_map_table`); `templates/handoff-story.md.j2` (Screens section); `skills/pm-prototype-html/SKILL.md` (anchor + hash-router requirement); `lib/artifact_contracts.py`'s `validate_prototype_html` (`SCREEN_ANCHOR_MISSING` check).
...
_Entries 41-43 recorded 2026-08-05: #41/#42 from the GH #63 (AdCept) engine follow-ups, #43 from an offline-install `pm_os_update` report. All fixed on `fix/handoff-update-polish` with tests. #37-40 land separately on `fix/gh-issues-59-62` (PR #64); numbering is global across the two open branches. Lands via commit → push → `pm_os_update.py`._
