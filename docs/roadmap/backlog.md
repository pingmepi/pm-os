...
## 18. 🟠 Backend engineering (TRD) has no code-enforced contract, unlike the front-end design spec

**Severity:** P3 — quality/parity gap, not a bug. The pipeline is fully usable without it.
**Status:** Open. Recorded 2026-07-21.

**Symptom:** The front-end/UX surface (stage 04 Design Spec) is a mandatory, section-validated stage: `lib/artifact_contracts.validate_artifact` enforces its required sections (Information Architecture, Key User Flows, Component Inventory, tokens, Accessibility, etc.) and the stage skill runs the validator in strict mode before generation telemetry. The backend/systems surface (stage 08 TRD) has **no equivalent**: `REQUIRED_SECTIONS` covers only stages 03–06, so `validate_artifact` returns `[]` for stage 08 immediately (`lib/artifact_contracts.py:579`). Its Architecture, Data Model, and API / Interface Contracts sections are purely prompt-driven — the agent generates them against the SKILL.md output spec, and nothing checks they are even present, let alone well-formed. A TRD could omit API/Interface Contracts entirely and pass every mechanical check.

**Consequence:** Backend design depth is opt-in and unenforced. A project can complete the whole core pipeline (01–07 approved) with zero backend design, and even an approved TRD carries no structural guarantee. This is an asymmetry: front-end/UX gets first-class, enforced treatment; backend engineering rides on prose + the agent's self-check in the prompt. The one exception is the Phase 3.5b Work Breakdown, whose `TSK-###` ids now get real machine validation via `/pm-check` (`lib/consistency.py`) — the rest of the TRD does not.

**Evidence:** `lib/artifact_contracts.py:126` (`REQUIRED_SECTIONS` = {03,04,05,06}) and `:579` (early `return []` when stage not in that set); contrast with the stage-04 required-section list. Surfaced 2026-07-21 while reviewing where Architecture / API-contract sections are built in the system (they are assumed, not built or validated).

**Proposed fix:** Give stage 08 the same treatment stages 03–06 have — add a `REQUIRED_SECTIONS["08"]` entry (System Context, Architecture, Data Model, API / Interface Contracts, Non-Functional Implementation, Rollout/Migration/Deployment, Work Breakdown, …) and a `_validate_stage_08` that at minimum checks required sections are present and non-empty, with the GenAI sections required when `genai_flag=true`. Optionally deepen it the way stage 03/06 traceability works: give API endpoints / data entities stable ids so a handoff (Phase 4b) could export backend work with the same rigor as PRD requirements and TRD tasks. Keep it WARNING-only first (existing TRDs on disk must keep passing), matching the v2-contract backward-compat convention.

---

_Recorded 2026-06-20 during v0.5.6 rollout testing (entries 1-3); entry 4 recorded 2026-07-09 (IMP-002); entries 5-9 recorded 2026-07-09 during a demo-project run (IMP-001, IMP-003 through IMP-006); entry 10 recorded 2026-07-14 while reviewing stage-05 slice selection; entries 11-13 recorded 2026-07-15 during a RepAssist v1.0.8→v1.0.10 dogfooding pass (IMP-007 through IMP-009), each verified against the current codebase before being logged; entries 14-17 recorded 2026-07-15 during a docs cleanup pass, migrated from `docs/archive/codex-pr-audit.md` (dated 2026-06-22, since archived) — of that audit's 11 originally-open items, 7 were re-verified as already fixed (folded into the archived doc's resolution note) and these 4 were re-verified as still open. Entry 18 recorded 2026-07-21 while reviewing where the TRD's Architecture / API-contract sections are built (found to be prompt-driven and unvalidated). All documentation-only unless noted. Changes above land via the normal commit → push → `pm_os_update.py` path; they are inert until then._
