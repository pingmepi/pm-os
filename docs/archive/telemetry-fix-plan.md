...
> **All areas shipped in v0.5.3 (2026-06-18).** Area 1: reliable central sync via `lib/git_sync.push_all` + `scripts/pm_sync.py` + `/pm-sync` skill with `--verify`; failures now loud. Area 2: real `model` id captured in `stage_generated`; `model_tier` derived from `config.model_tier_for_stage`. Area 3: `time_to_approve_seconds`, `char_edit_distance`, `normalized_edit_distance` recorded at approval via `lib/text_metrics.py`; `semantic_distance` as optional agent estimate. Area 4: `feedback_submitted` enters the hash-chained telemetry stream. Area 5: `pm_os_verify.py` includes a telemetry self-test. The revalidation note below was accurate as of 2026-06-17 (before v0.5.3 landed).
>
> Moved to `docs/archive/` 2026-07-15 (nothing left actionable) — retained for provenance. Note: [backlog #13 (IMP-009)](../roadmap/backlog.md) opens a *new*, distinct telemetry gap (missing per-stage token counts) discovered after this plan shipped — not a reopening of any area here.
