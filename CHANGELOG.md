...
### Fixed
- **Screen map counts journey-only coverage as covered ([PR #41](https://github.com/pingmepi/pm-os/pull/41), Codex review on #40).** `reference/screen-map.md` derived its "stories with no screen" list from each screen's literal `Serves:` ids, so a story covered only through a journey link (screen serves `UJ-001`, and `UJ-001` serves `US-001`) was reported uncovered even though its own story file listed the screen via journey resolution — the two views contradicted the docstring's "can never disagree" promise. Coverage now comes from `covered_story_ids`, captured during the same per-story pass that resolves screens over requirements *and* journeys, so the map and the story files are computed from one resolution.
...
### Added
- **Screens are mapped to user stories in the handoff package (tech-lead feedback).** A developer opening a story file could see its requirements, journeys and test cases but nothing about *which screens it touches* — screens lived only as prose, so the package never read the design spec at all. The stage-04 Information Architecture now declares each screen as a stable **`SCR-###`** with a `Serves:` line citing the `US-###`/`FR-###`/`UJ-###` ids it supports — the design half of the same spine `US`/`FR`/`UJ`/`TC`/`TSK` already form. `.traceability.yaml` (now **schema v3**) indexes them under a `screens:` map with reverse requirement→screen links and a reserved `design_refs: []` slot for a later design export; only an **approved** stage 04 contributes, and parsing is scoped to `## Information Architecture` so a `SCR-###` cited under Key User Flows stays a reference. `/pm-share --package` renders a **"Screens this story touches"** section per story plus `reference/screen-map.md` (the reverse view: each screen, what it serves, and any story no screen covers). `/pm-check` validates screen ids are unique (duplicate = error) and sequential, that each traces to a real PRD id, and that every user story is served (gap/orphan/unknown-id/coverage = warnings). Everything degrades gracefully: a project with no `SCR-###` ids still packages, showing `— not captured in source —`.
- **Offline Jira export — `/pm-handoff jira --offline` (tech-lead feedback).** Creating tickets used to require an authorized Atlassian connector, which not every PM has. `pm_handoff.py export` runs the *same* plan builder (same approval gate, same US→Epic / FR→Story / TSK→Task mapping) and writes `handoff/jira-import.csv` plus a field-mapping guide, entirely offline. Parent links use the importer's `Issue Id`/`Parent Id` pair so the epic hierarchy survives; each row carries its stable id as a column and as a `pm-os-<id>` label, so after the import the created keys can be recovered from Jira and fed to the existing `pm_handoff.py record` — the offline route ends in the same state as the connector route. The synthetic `UNASSIGNED` epic is never emitted as a ticket; its children export parentless, matching the connector path.
- **`lib/jira_markup.py` — Markdown → Jira wiki markup.** Ticket descriptions are converted so they render in Jira instead of arriving with literal `##`/`**`. A documented subset (headings, emphasis, strikethrough, lists with nesting, links, tables, blockquotes, rules, inline/fenced code); unrecognized syntax passes through unchanged, and code spans/blocks are protected from every other rule.
...
### Added
- **Phase 4b — Jira handoff export (`/pm-handoff jira`).** Export the approved pipeline to Jira, gated on approved status and keyed to PM-OS's stable ids. `scripts/pm_handoff.py plan` parses the approved PRD (+ approved TRD) into a tracker-agnostic ticket map — each `US-###` → epic, its `FR-###`/`REQ-###` → child story, each approved `TSK-###` → child task under the owning epic (unowned items go to an Unassigned bucket) — and writes a PM-readable dry-run (`handoff/jira-plan.md`) plus a machine map (`.json`), fully offline. The `skills/pm-handoff/` skill orchestrates **dry-run → PM confirms → create via the Atlassian MCP → record**; `pm_handoff.py record` writes each created ticket key into the matching requirement's/task's `tickets: []` slot in `.traceability.yaml` and logs a new `handoff_exported` telemetry event (refs/counts/keys only — never bulk external data). Tickets are created through the runtime's Atlassian connector (authorized in connector settings / `/mcp`); PM-OS never handles tokens. Phase 4a (the local, offline handoff package) already shipped as `/pm-share --package` in v1.0.12.
- **Phase 3.5b — TRD task IDs (`TSK-###`).** The stage-08 TRD contract now requires a **Work Breakdown** section that decomposes the build into discrete tasks, each with a stable `TSK-###` id and an `Implements:` trace to the PRD requirement(s) (`US-###`/`FR-###`/`REQ-###`) it delivers. `lib/artifact_contracts.py` gains `TASK_ID_RE`, `split_task_blocks`, `task_implements`, and `task_id_declarations`. `.traceability.yaml` (now **schema v2**) indexes tasks under a `tasks:` map with reverse requirement↔task links and a reserved `tickets: []` slot, rebuilt on stage-08 approval; the index is derived, so a v1 file upgrades transparently on the next rebuild. Only an **approved** TRD's tasks are indexed (a draft/stale TRD contributes nothing, so re-approving an upstream stage drops obsolete links), and task parsing is scoped to the `## Work Breakdown` section so a stray `TSK-###` elsewhere in the TRD is never indexed or exported. New resolver queries (`tasks_for_requirement`, `requirements_for_task`) and a `pm_trace.py task` subcommand. `/pm-check` validates task ids are unique (duplicate = error) and sequential, that each traces to a real PRD requirement, and that every PRD requirement is implemented by at least one task (gap/orphan/unknown-req/coverage = warnings). Prerequisite for the Phase 4b tracker export.
