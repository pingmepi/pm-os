...
| `skills/pm-interview` → `scripts/pm_interview.py` | Standalone re-run of the discovery interview against the still-open `00-context/known-unknowns.md` gaps. `list-unknowns` emits the open gaps (JSON) for the skill to ask; the skill registers answers via `pm_context_import.py record-interview` (reused unchanged), then `resolve` marks each answered gap `[resolved: <source> <date>]` **in place** (never deleted), credits the PM interview source, and logs `interview_conducted` with `mode: rerun`. Honors `--interview-answers`/`PM_OS_INTERVIEW=skip`/non-tty; never self-approves. |
