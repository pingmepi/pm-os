...
1. **Prepare final frontmatter and body.** Generate the body first, then prepare final frontmatter with the values below. Use the final frontmatter and body for `01-brief.md`; the snapshot helper copies that exact artifact into `.history/` after it is written.
...
3. **Write `01-brief.md`** with the same frontmatter:
   ```yaml
   ---
   stage: 01-brief
...
```bash
python3 ~/.pm-os/scripts/pm_snapshot.py 01
```
