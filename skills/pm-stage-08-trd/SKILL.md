...
1. **Prepare final frontmatter and body.** Generate the body first, then prepare final frontmatter with the values below. Use the final frontmatter and body for `08-trd.md`; the snapshot helper copies that exact artifact into `.history/` after it is written.
...
3. **Write `08-trd.md`** with the same frontmatter:
   ```yaml
   ---
   stage: 08-trd
...
```bash
python3 ~/.pm-os/scripts/pm_snapshot.py 08
```
...
5. **Validate the artifact contract:**
   ...
   ```
   Section-shape findings are warnings only. If validation exits non-zero, repair `08-trd.md`, recompute `generated_hash`, rerun the snapshot helper, and rerun validation. Do not update metadata or log `stage_generated` until validation passes.
