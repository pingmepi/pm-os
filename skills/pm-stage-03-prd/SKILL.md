...
<List the core user stories in priority order using stable IDs such as `US-001`. **Tier every story** by carrying the release band from the approved scope as a `- **Tier:** <mvp|v1|v2|later>` field. **Fidelity follows tier (D2):** `mvp` stories are full self-contained mini-specs (all fields below) so the readable handoff can render them without invention; `v1`/`v2`/`later` stories are **SOW-grade stubs** — enumerated so the whole product is visible, but held to only **Story**, **Epic**, **Tier**, **Value** (one line), **Size** (rough estimate), **Rationale**, **Depends on**, **Acceptance intent** (one line), and **Traceability**. Do **not** author Happy path, Edge cases, Key UI steps, or full acceptance criteria for a stub — those are written when it is promoted to `mvp` (`/pm-promote`). For each **`mvp`** `US-###` include the full mini-spec:
...
- **Tier** — the release band from the approved scope (`mvp` | `v1` | `v2` | `later`); default `mvp`.
...
- User Stories with Acceptance Criteria must be testable, prioritized, and cover the critical flows needed for launch. Each **`mvp`** story should be a self-contained mini-spec with explicit Happy path, Edge cases / alternate paths, data fields, key UI steps with per-step system process + acceptance + corner cases/exceptions, and traceability so the handoff can render it without invention. Non-mvp (`v1`/`v2`/`later`) stories are SOW-grade stubs (Value, Size, Rationale, Depends on, Acceptance intent) and are elaborated to full fidelity only on promotion.
