...
- **Read each source directly with the right tool for its format.** `.md` / `.txt` are read verbatim. For binary formats, reach for your runtime's matching skill/tool before falling back to anything generic — Claude Code reads PDFs directly and ships `pdf`, `docx`, `pptx`, and `xlsx` skills:
  - `.pdf` / `.docx` → native file reading or the `pdf` / `docx` skill.
  - `.pptx` / `.ppt` → the **`pptx` skill** (python-pptx), which extracts slide text, tables, and speaker notes cleanly — text-bearing decks are **not** lossy. Only reach for the multimodal/screenshot path when a slide's meaning lives in a diagram, screenshot, or layout that text extraction can't carry.
  - `.xlsx` / `.xls` / `.csv` → the **`xlsx` skill** (or read tabular text directly).
  If your runtime lacks a matching skill, convert with an available tool (`pandoc`, `pdftotext`, `libreoffice --headless`); only as a last resort ask the PM to export to Markdown.
- **Flag lossy extraction — but only when it's actually lossy.** Text-based PDFs, `.docx`, and text-bearing `.pptx`/`.xlsx` read via their skill extract cleanly and are **not** lossy. A source is lossy when its meaning can't be carried by text extraction: a **scanned/image-only PDF** (no selectable text), a **table-heavy / multi-column** layout that scrambles on extraction, or a **deck/image whose content is diagram- or screenshot-borne**. When a source is genuinely lossy, emit an `FYI:` calling it out and lean on the review gate: e.g. `FYI: payments-spec.pdf looks scanned/table-heavy — extraction may be imperfect; review the wiki section sourced from it carefully before approving.` The script pre-tags decks/sheets/images `extraction_quality: unverified` at registration; after you read one, resolve it to `clean` (skill read the text fine) or `lossy` (content was image/diagram/layout-borne) rather than leaving the provisional tag to stand as a verdict. Never silently treat a degraded extraction as faithful, and never leave a cleanly-read deck flagged lossy.
