...
```bash
python3 ~/.pm-os/scripts/pm_enhance.py start --ask-file <pm-authored-ask.md> [--codebase <path-or-url>] [--ref <git-ref>] [--subpath <monorepo-subpath>]
```
...
For remote codebases, preparation may clone only into PM-OS-owned project storage. For local repositories, the start/scan path records before-state evidence and must leave the repository unchanged.
