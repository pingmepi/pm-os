...
- In Claude, invoke PM-OS skills with slash commands such as `/pm-new`,
  ...
  `/pm-status`, `/pm-feedback 03`, `/pm-handoff`, `/pm-sync`, and `/pm-os-verify`.
  (`/pm-handoff` is the single export skill — `--raw` text, `--package` per-audience
  handoff folders, or Jira tickets.)
- In Codex, invoke PM-OS skills through the `/skills` picker or by mentioning the
  ...
  `$pm-approve`, `$pm-stage-01-brief`, `$pm-status`, `$pm-feedback`, `$pm-handoff`,
  `$pm-sync`, and `$pm-os-verify`.
